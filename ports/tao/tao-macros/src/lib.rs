...
impl Parse for AndroidFnInput {
  fn parse(input: ParseStream) -> syn::Result<Self> {
    ...
    let ret = if !input.is_empty() && !input.peek(syn::token::Bracket) {
    ...
    if !input.is_empty() {
      return Err(input.error("unexpected trailing tokens in android_fn input"));
    }
    Ok(Self {
      domain,
      package,
...
/// 5. List of extra types your Rust function expects. Pass empty array if the function doesn't need any arguments.
...
impl Parse for GeneratePackageNameInput {
  fn parse(input: ParseStream) -> syn::Result<Self> {
    ...
    if !input.is_empty() {
      return Err(input.error("unexpected trailing tokens in generate_package_name input"));
    }
